...
async def get_count_by_link(username:str, start_time:dt=None, end_time:dt=None, db:session=Depends(get_db)):
	...
	try:
		...
		click_count = db.execute('SELECT ClicksResample.click_count, Link.link_name FROM ClicksResample, Link WHERE Link.id = ClicksResample.link_id AND link_id in :link_list AND click_sampled_timestamp BETWEEN :start AND :end;', { 'link_list': tuple(x['id'] for x in links), "start": start_time, "end": end_time })
