...
async def create(request:RequestView, db:session=Depends(get_db)):
	"""Async API to create view

	Args:
		request (RequestView): Serialized request data
...
async def get(id:int=None, db:session=Depends(get_db)):
	"""Async API to get views

	Args:
		id (int, optional): view id. Defaults to None.
		db (session, optional): DB connection session for db functionalities. Defaults to Depends(get_db).
...
async def update(request:UpdateView, id:int=None, db:session=Depends(get_db)):
	"""Async API to update view

	Args:
		request (UpdateView): View attributes to be updated
		id (int, optional): view id. Defaults to None.
		db (session, optional): DB connection session for db functionalities. Defaults to Depends(get_db).
...
async def delete(id:int=None, db:session=Depends(get_db)):
	...
	try:
		if id:
			...
			return JSONResponse(content={"message": f"{deleted_rows} views deleted"}, status_code=status.HTTP_200_OK)
	except Exception as e:
		return JSONResponse(content={"message": str(e)}, status_code=status.HTTP_400_BAD_REQUEST)
