...
def bulk_create_view(db:session, views:List[ViewSchema]):
	"""Function to create views in bulk

	Args:
		db (session): DB connection session for ORM functionalities
		views (ViewSchema): Serialized views

	Returns:
		orm query set: returns created views
	"""
	_views = [
		ViewsResample(
			...
			view_sampled_timestamp=view["view_sampled_timestamp"],
			profile_id=view["profile"]
		)
		for view in views
	]
	db.add_all(_views)
