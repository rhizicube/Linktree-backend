...
from schemas.clicks_resample import ClicksResampleSchema as ClickSchema
...
def bulk_create_click(db:session, clicks:List[ClickSchema]):
	"""Function to create clicks in bulk

	Args:
		db (session): DB connection session for ORM functionalities
		clicks (ClickSchema): Serialized clicks

	Returns:
		orm query set: returns created clicks
	"""
	_clicks = [
		ClicksResample(
			click_count=click["click_count"],
			click_sampled_timestamp=click["click_sampled_timestamp"],
